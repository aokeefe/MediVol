...
def get_items(data):
    if re.match("^.*:.*\Z", data.strip()) is not None:
        ...
        try:
            ...
            print '[ERROR] With box name ' + box_name_text
            ERROR_COUNT = ERROR_COUNT + 1
            return None
        ...
        if re.match("^(.*;)*.*\Z", items):
            ...
            for item in item_list:
                if re.match('^\d+ [a-z A-Z/&:\'\.]+\Z', item.strip()):
                    ...
                    print '[ERROR] reading items ' + items
                    ERROR_COUNT = ERROR_COUNT + 1
                    return None
...
def importer(file_name):
    ...
    for line in csvArray[line_count:]:
        ...
        if valid:
            try:
                box = Box(box_id=line_data[0], box_category=line_data[1], box_size=line_data[2], weight=line_data[3], old_expiration=line_data[5])
                box.save()
